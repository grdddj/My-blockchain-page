...
async function buy(coinSymbol) {
  ...
  if (!confirm(`Opravdu si chcete zakoupit token ${coinSymbol}?`)) {
    return
  }
  ...
  try {
    ...
    .on('receipt', function(receipt) {
      ...
      alert("Děkujeme za nákup tokenu " + coinSymbol + "!")
    ...
    .on('error', function(error) {
      $("#error").html('Error - je Metamask připojen ke správné adrese? : ' + accounts[0]);
    })
  } catch (error) {
    console.log ('error',error);
    alert("Transakce se nezdařila a token nebyl zakoupen -  zkuste to prosím znovu!");
    loaderTX.hide();
    mainContent.show();
  }
}
